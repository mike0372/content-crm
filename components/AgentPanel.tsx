...
function flattenDiff(before: Record<string, unknown>, after: Record<string, unknown>) {
      ...
      <div className="divide-y divide-white/[0.05] px-4 py-1">
        {rows.map(({ key, before, after }) => (
          <div key={key} className="py-2.5 text-[12px]">
            <div className="mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-zinc-500">{key}</div>
...
function ChatBubble({ msg, onApprove, onReject }: {
  msg: Message;
  onApprove: () => void;
  onReject: () => void;
}) {
...
const WELCOME: Message = {
  role: "assistant",
  content: "Hi! I'm your AI assistant. Ask me anything about your content, or tell me what you'd like to change — I'll ask a couple of questions to make sure I get it right.",
  agentType: "message",
};
...
export function AgentPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function sendMessage() {
    ...
    try {
      // Build conversation history for the API (role + content pairs only)
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Something went wrong. Please try again.", agentType: "message" },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  function handleReject(msgIndex: number) {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "assistant", content: "No problem — change cancelled. What would you like to do instead?", agentType: "message" },
    ]);
  }
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 right-0 z-40 flex w-[420px] flex-col border-l border-white/[0.09] bg-[rgba(9,16,32,0.72)] backdrop-blur-[48px] backdrop-saturate-[160%] shadow-[-8px_0_40px_rgba(0,0,0,0.4)] transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]",
      ...
      >
        ...
        <div className="flex h-16 flex-shrink-0 items-center gap-3 border-b border-white/[0.08] px-5">
          ...
          <div className="leading-tight">
            <div className="text-[13px] font-bold text-white">AI Agent</div>
            <div className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#06b6d4]">
              Haiku · Full DB access
            </div>
          </div>
