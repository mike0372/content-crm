...
- Tables (schema in Supabase migrations `init_content_crm_schema`):
  - `content_items` — unified ContentItem store (`stage` = "idea" | "production"); scalar columns + JSONB for nested fields (`hook`, `script`, `captions`, `engagement`, `checklist`, `results`, `demand_signal`, `status_history`). `updated_at` auto-set by trigger. `instagram_media_id` (text, nullable) binds an item to its posted reel so live IG metrics flow back into the editor (see Linked Reel below). `focused` (boolean, default false; migration `add_focused_to_content_items`, already applied) is the user-picked "focus" flag — toggled via a small check button on Board cards and Ideas cards, rendered as an orange outline; it rides the normal item PUT (`apiSaveVideo`/`apiSaveIdea`), so it survives promote and syncs across tabs via polling. `priority` (double precision) is the board's intra-column sort key — **lower sorts higher** (top of column); defaults to the createdAt epoch so legacy order is preserved. Board drag reorders within a column (and across columns) by writing a fractional midpoint priority to the dragged card only; when repeated reorders shrink the gap to float precision the whole column is re-spaced to clean integers. Board drag-saves use an opt-in optimistic-concurrency guard — `apiSaveVideo(item, expectedUpdatedAt)` → PUT `/api/videos/[id]` returns 409 + current row → `ConflictError`, so a stale cross-device move reverts (toast) instead of clobbering. In-editor autosave deliberately omits the check (keeps last-write-wins) to avoid false conflicts on its stale baseline.
  ...
  - `instagram_account` + `instagram_posts` — full Meta Graph API data (account stats + every fetched post field incl. insights). `instagram_posts.avg_watch_time` (numeric, seconds) holds the `ig_reels_avg_watch_time` reels insight, fetched in a **separate** Graph call so a failure can't zero the core metrics. Stale posts (outside the latest media window) are pruned on each sync — but their history survives in `instagram_post_snapshots`.
  - `instagram_post_snapshots` — append-only, one row per post per sync; preserves full metric history across the 50-window prune and powers the `/performance` time-series chart. Migration: `supabase/migrations/20260630_add_instagram_post_snapshots.sql` (run manually; best-effort write — sync still works without it).
  - `backups` — daily full-snapshot JSON bundles written by the cron (last 14 kept). Migration: `supabase/migrations/20260630_add_backups.sql` (run manually; optional — manual Export/Import works without it).
...
- `lib/instagram.ts` — Graph API sync (`getInstagramCache`, `syncInstagram`) reads/writes the IG tables. `syncInstagram` reads the active token via `lib/instagramToken.ts` and throws a typed `INSTAGRAM_TOKEN_INVALID` on Meta error code 190 / HTTP 401. On each sync it **zero-guards** insight counters (keeps `max(fresh, stored)` since they're lifetime-cumulative, so a failed Graph call can't overwrite good metrics) and appends a per-post row to `instagram_post_snapshots`; `getMetricHistory()` aggregates those into the `/performance` daily time-series chart.
...
- `lib/ai.ts` — central Claude config + the single choke point for every AI call. `MODELS` (override via `ANTHROPIC_MODEL_FAST`/`ANTHROPIC_MODEL_SMART`), `createMessage(params, {route, tier})` (selects the model, enforces optional `AI_MONTHLY_USD_BUDGET`, logs tokens+est. cost to `ai_usage`, and **strips unpaired UTF-16 surrogates** from system/string message content — a `.slice()` that cuts an emoji in half otherwise makes the request body invalid JSON and the API 400s the whole call), `getMonthlyAiUsage()` for the cost panel. **All AI routes call through this — never instantiate the Anthropic SDK directly in a route.**
- **Upload date (calendar placement) API** — an item's upload date is *which* `calendars[week].days[day]` array holds its id, not a column on the item. `GET/PUT /api/videos/[id]/schedule` (`{date: "YYYY-MM-DD" | null}`) reads/moves that placement across weeks (`lib/data.ts` `findScheduledDate`/`scheduleItemOnDate`; date↔week/day helpers `mondayOfIsoWeek`/`dayKeyOf`/`parseYMD`/`dateOfSlot` in `lib/week.ts`). The video editor's META tab has an "Upload date (calendar)" field (`ScheduledDateField` in `VideoEditor.tsx`) that uses it; the Calendar page picks changes up via its normal polling.
...
- `app/api/agent/route.ts` — **Edward** (the conversational CRM agent). Sends Claude a **compact index** of every item (id/title/status/pillar) plus **full detail only for items relevant to the latest question** (keyword retrieval, top 6), and **bounds replayed history** to the last 12 turns — so prompt size and cost stay roughly flat as the library and chat grow (it previously injected the entire DB on every message). Conversation history is still persisted in full client-side (`agent_conversations`).
- `lib/export.ts` — own-your-data: `buildExportBundle`/`importBundle` (full JSON snapshot of content + calendars + performance + IG cache; restore is an additive upsert, never deletes), `buildReelsCsv`, and `writeBackup` (cron-driven, prunes to last 14). Routes: `GET /api/export` (`?format=csv` for a reels spreadsheet), `POST /api/import`. UI: dashboard "Your data" card (`components/dashboard/DataCard.tsx`).
